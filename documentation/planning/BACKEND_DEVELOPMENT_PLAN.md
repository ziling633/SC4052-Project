...
> **Detailed specifications:** See [PHASE_1_CORE_API.md](../phases/PHASE_1_CORE_API.md#-detailed-tasks--checkpoints) for complete data model documentation with examples and validation rules.

**Overview:**
- **Canteens**: Static reference table (cantee_id, name, location, lat, lng)
- **Reports**: Temporal events (report_id, canteen_id, crowd_level, timestamp, source)
- **Computed Status**: Derived from reports (not persisted) - aggregated crowd level with confidence scores
...
> **Full API specification:** See [PHASE_1_CORE_API.md](../phases/PHASE_1_CORE_API.md#-rest-api-endpoints) for complete endpoint documentation with request/response examples and status codes.

**Core Endpoints:**
1. `POST /api/v1/report` - Submit crowd report
2. `GET /api/v1/canteens/status` - Fetch all canteen statuses
3. `GET /api/v1/canteens/{id}` - Fetch single canteen details
4. `GET /api/v1/health` - Health check

**Interactive API Docs:** Available at `/docs` (SwaggerUI) when backend is running.
```
